...
function onDeviceReady() {
	get_notifications();
	checkConnection();
	setInterval(function(){get_notifications();},15000);
	setInterval(function(){checkConnection();},30000);
}

jQuery(document).ready(function(e) {
	
	jQuery('body').delegate('.logout','tap',function(){
		loading_ajax();
...
function get_notifications(){
	if (localStorage.getItem('wordpress_loggedin')) {
		jQuery.ajax({
			...
			success: function (data) {
				dat_ = jQuery.parseJSON(data);
				if( dat_.notificaciones ){
					for(i=0;i < dat_.notificaciones.length; i++){
						//console.log(dat_);
						navigator.notification.alert(dat_.notificaciones[i].message, function(){}, dat_.notificaciones[i].title,'Aceptar');
						navigator.notification.vibrate(1000);
					}
				}
			}
